  ...
  module V1
    class CartsController < ActionController::API
      ...
      def add_item
        ...
        begin
          add_product_to_cart_service.call(
            client_id: client_id,
          ...
          render json: { "successful": true, "status": 200, message: 'Product adicionado ao carrinho com sucesso!', cart: @cart.to_h_for_display },
                 status: :ok
          ...
          render json: { "successful": false, "status": 500, errors: 'Um erro inesperado aconteceu.' },
      ...
      def payment_status
        ...
        when /pago/
          render json: { "successful": true, "status": 200, msg: "Pedido nº #{@cart.id}  enviado para cozinha!", payment_status: @cart.payment_status },
                 status: :accepted
        else
          render json: { "successful": true, "status": 200, id: @cart.id, payment_status: @cart.payment_status },
                 status: :ok
        end
      end
      ...
      def create_payment
        ...
        if publisher.publish('CarrinhoFinalizado', @cart.to_h_payment_display)
          render json: { "successful": true, "status": 200, id: @cart.id, payment_status: 'creating_payment' },
                 status: :ok
        end
      rescue ArgumentError => e
        render json: { "successful": false, "status": 422, errors: e.message }, status: :unprocessable_entity
      ...
      def remove_item
        if cart_params[:product_id].blank?
          return render json: { "successful": false, "status": 400, errors: 'Parâmetros faltantes: product_id' },
                        status: :bad_request
        end
        ...
        begin
          remove_product_from_cart_service.call(
            client_id: @cart.client_id,
            product_id: cart_params[:product_id],
            quantity: cart_params[:quantity],
            cart: @cart
          ...
          render json: { "successful": true, "status": 200, msg: 'Produto removido do carrinho com sucesso!' },
                 status: :accepted
        rescue ArgumentError => e
          render json: { "successful": false, "status": 422, errors: e.message }, status: :unprocessable_entity
        rescue StandardError => e
      ...
      def update_item
        if cart_params[:product_id].blank? || cart_params[:quantity].blank?
          render json: { "successful": false, "status": 400, errors: 'Parâmetros faltantes: product_id, quantity' },
                 status: :bad_request
          return
        end
        ...
        begin
          update_product_quantity_in_cart_service.call(
            client_id: @cart.client_id,
            product_id: cart_params[:product_id],
      ...
      def update_status_in_progress_orders
        if cart_params[:progress_status].blank?
          return render json: { successful: false, status: 400, errors: 'Parâmetros faltantes: progress_status' },
                        status: :bad_request
        end
        if cart_params[:cart_id].blank?
          return render json: { successful: false, status: 400, errors: 'Parâmetros faltantes: cart_id' },
                        status: :bad_request
        end
        ...
        begin
          ...
          render json: { successful: true, status: 200, msg: 'Status atualizado!', cart: updated_cart.to_h_for_display },
                 status: :ok
        rescue ArgumentError => e
          case e.message
      ...
      def set_cart
        ...
        begin
          ...
          render json: { "successful": false, "status": 500, errors: 'Um erro inesperado aconteceu ao configurar o carrinho' },
                 status: :internal_server_error
        end
      end
